...
func withAuthCookieForTest(t *testing.T, h *Handler, req *http.Request, sessionCode string) *http.Request {
	...
	token, err := h.auth.MintToken(sessionCode, time.Now().Add(time.Hour))
	if err != nil {
...
func TestHandleVerifySession_ErrorMappings(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			body:       `{"session_code":"AP-AAAA-BBBB","pin":"482917"}`,
		...
		},
		{
			...
			body:       `{"session_code":"AP-AAAA-BBBB","pin":"000000"}`,
...
func TestHandleVerifySession_SuccessContract(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/session/verify", strings.NewReader(`{"session_code":"AP-7K9X-M2NF","pin":"482917"}`))
	rr := httptest.NewRecorder()
	h.HandleVerifySession(rr, req)
...
func TestHandleVerifySession_LocksOutAfterTooManyFailures(t *testing.T) {
	...
	firstReq := httptest.NewRequest(http.MethodPost, "/api/session/verify", strings.NewReader(`{"session_code":"AP-AAAA-BBBB","pin":"000000"}`))
	...
	secondReq := httptest.NewRequest(http.MethodPost, "/api/session/verify", strings.NewReader(`{"session_code":"AP-AAAA-BBBB","pin":"000000"}`))
	...
	thirdReq := httptest.NewRequest(http.MethodPost, "/api/session/verify", strings.NewReader(`{"session_code":"AP-AAAA-BBBB","pin":"482917"}`))
...
func TestHandleVerifySession_SuccessResetsFailureCount(t *testing.T) {
	...
	badReq := httptest.NewRequest(http.MethodPost, "/api/session/verify", strings.NewReader(`{"session_code":"AP-AAAA-BBBB","pin":"000000"}`))
	...
	goodReq := httptest.NewRequest(http.MethodPost, "/api/session/verify", strings.NewReader(`{"session_code":"AP-AAAA-BBBB","pin":"482917"}`))
	...
	badAgainReq := httptest.NewRequest(http.MethodPost, "/api/session/verify", strings.NewReader(`{"session_code":"AP-AAAA-BBBB","pin":"000000"}`))
...
func TestHandleVerifySession_FailedAttemptLimiterIsScopedBySessionAndIP(t *testing.T) {
	...
	first := httptest.NewRequest(http.MethodPost, "/api/session/verify", strings.NewReader(`{"session_code":"AP-AAAA-BBBB","pin":"000000"}`))
	...
	second := httptest.NewRequest(http.MethodPost, "/api/session/verify", strings.NewReader(`{"session_code":"AP-AAAA-BBBB","pin":"000000"}`))
	...
	otherIP := httptest.NewRequest(http.MethodPost, "/api/session/verify", strings.NewReader(`{"session_code":"AP-AAAA-BBBB","pin":"000000"}`))
	...
	otherSession := httptest.NewRequest(http.MethodPost, "/api/session/verify", strings.NewReader(`{"session_code":"AP-CCCC-DDDD","pin":"000000"}`))
...
func TestHandleVerifySession_PerIPTokenBucketLimiter(t *testing.T) {
	...
	first := httptest.NewRequest(http.MethodPost, "/api/session/verify", strings.NewReader(`{"session_code":"AP-AAAA-BBBB","pin":"482917"}`))
	...
	second := httptest.NewRequest(http.MethodPost, "/api/session/verify", strings.NewReader(`{"session_code":"AP-AAAA-BBBB","pin":"482917"}`))
...
func TestHandleCheckAccess_Behavior(t *testing.T) {
	t.Parallel()
	...
	wrapped := shared.RequireSessionAuth(h.auth, h.HandleCheckAccess)

	t.Run("accepts a matching authenticated session", func(t *testing.T) {
		req := httptest.NewRequest(http.MethodGet, "/api/session/access?session_code=AP-7K9X-M2NF", nil)
		req = withAuthCookieForTest(t, h, req, "AP-7K9X-M2NF")
		rr := httptest.NewRecorder()
	...
	t.Run("rejects missing auth cookie", func(t *testing.T) {
		req := httptest.NewRequest(http.MethodGet, "/api/session/access?session_code=AP-7K9X-M2NF", nil)
		rr := httptest.NewRecorder()
	...
	t.Run("rejects mismatched session code", func(t *testing.T) {
		req := httptest.NewRequest(http.MethodGet, "/api/session/access?session_code=AP-AAAA-BBBB", nil)
		req = withAuthCookieForTest(t, h, req, "AP-7K9X-M2NF")
		rr := httptest.NewRecorder()
		...
		var got shared.ErrorResponse
		decodeJSONBody(t, rr, &got)
		if got.Code != "SESSION_MISMATCH" {
			t.Fatalf("code = %q, want SESSION_MISMATCH", got.Code)
		}
	})

	t.Run("requires a session code query parameter", func(t *testing.T) {
		req := httptest.NewRequest(http.MethodGet, "/api/session/access", nil)
		req = withAuthCookieForTest(t, h, req, "AP-7K9X-M2NF")
		rr := httptest.NewRecorder()

...
func TestHandleStart_RejectsInvalidLanguage(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/interview/start", strings.NewReader(`{"session_code":"AP-7K9X-M2NF","language":"fr"}`))
...
func TestHandleAnswerAsync_ValidationAndIdempotencyErrors(t *testing.T) {
	...
	t.Run("body_over_10kb", func(t *testing.T) {
		...
		body := `{"session_code":"AP-7K9X-M2NF","answer_text":"` + answer + `","question_text":"Q","turn_id":"turn-1","client_request_id":"req-1"}`
...
func TestHandleAnswerJobStatus_ValidationAndContract(t *testing.T) {
	...
	t.Run("job_not_found", func(t *testing.T) {
		...
		req := httptest.NewRequest(http.MethodGet, "/api/interview/answer-jobs/job-1?session_code=AP-7K9X-M2NF", nil)
		req.SetPathValue("jobId", "job-1")
		req = withSessionAuthContextForTest(req, "AP-7K9X-M2NF")
		rr := httptest.NewRecorder()

...
func setBaseValidEnv(t *testing.T) {
	...
	env := map[string]string{
		...
		"AI_MAX_TOKENS":                               "1024",
		"AI_LAST_Q_SECONDS":                           "30",
		"AI_CLOSING_SECONDS":                          "15",
		"AI_MIDPOINT_AREA_INDEX":                      "3",
...
func TestLoad_ParsesNewAsyncAndRateLimitKnobs(t *testing.T) {
	...
	if cfg.AsyncAnswerWorkers != 7 ||
		...
		cfg.AsyncAnswerRecoveryEvery != 11 ||
		cfg.AsyncAnswerStaleAfterS != 240 ||
		cfg.AsyncAnswerJobTimeoutS != 200 {
	...
	if cfg.VerifyIPRatePerMinute != 120 ||
		...
		cfg.VerifyFailWindowS != 700 ||
		cfg.VerifyFailLockoutS != 950 {
		t.Fatalf("unexpected verify limiter config: %#v", cfg)
	}

	if cfg.VoiceIPRatePerMinute != 40 ||
		cfg.VoiceIPBurst != 7 ||
		cfg.VoiceSessionRatePerMin != 8 ||

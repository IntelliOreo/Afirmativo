...
const (
	...
	deepgramGrantLegacyPath = "/v1/auth/tokens/grant"
...
// TokenGrantResponse is normalized from Deepgram token grant response.
...
// Client calls a Deepgram-compatible token grant API.
type Client struct {
	baseURL  string
	apiKey   string
	model    string
...
func (c *Client) MintToken(ctx context.Context, ttlSeconds int) (*TokenGrantResponse, error) {
	...
	candidates := []string{deepgramGrantPath, deepgramGrantLegacyPath}
	var lastErr error

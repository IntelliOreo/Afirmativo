...
type reportInterviewProviderStub struct {
	areas       []QuestionAreaRow
	answers     []AnswerRow
	answerCount int
}

func (s *reportInterviewProviderStub) GetAreasBySession(context.Context, string) ([]QuestionAreaRow, error) {
	return s.areas, nil
}

func (s *reportInterviewProviderStub) GetAnswersBySession(context.Context, string) ([]AnswerRow, error) {
...
func TestServiceGetOrGenerateReport_LanguageTranscriptFlowAndBilingualReport(t *testing.T) {
	...
	const (
		...
		evaluationJSON   = `{"current_criterion":{"evidence_summary":"English evidence summary","recommendation":"follow_up"}}`
